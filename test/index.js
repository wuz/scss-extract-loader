...
describe("sass-vars-loader", () => {
  it("should succesfully compile basic variables", () => {
    return runTest("./scss/basic.scss").then((results) => {
      ...
      expect(Array.from(stats.compilation.fileDependencies)).to.include(source);
    });
  });

  it("should succesfully compile nested variables", () => {
    return runTest("./scss/nested.scss").then((results) => {
      const compiled = results.compiled;
      const stats = results.stats;
  ...
  it("should succesfully include files from includePaths", () => {
    return runTest("./scss/include.scss", {
      ...
      const sourceIncluded = path.resolve(
        __dirname,
        "./scss/include/included.scss"
      );

      expect(
        Array.from(stats.compilation.fileDependencies)
      ).to.include.members([sourceRoot, sourceIncluded]);
    });
  });

  it("should succesfully use minimal plugin", () => {
    return runTest("./scss/basic.scss", { plugins: ["minimal"] }).then(
      (results) => {
        const compiled = results.compiled;
        const stats = results.stats;
